...
class PushingEnv(gym.Env):
    ...
    def __init__(self, graphics = False, seed = None, fps = 30, randomize_shapes = True, shapes = None):
        ...
        self.BOX_Y_WIDTH = 0.02  # Changed from 0.01 to 0.02 for wider variation (±0.01)

        self.BOX_X_CENTER = 0.1
        self.BOX_X_WIDTH = 0.02  # Changed from 0.01 to 0.02 for wider variation (±0.01)
        ...
        self.SHAPE_RADIUS_WIDTH = 0.01   # Variation in radius
    ...
    def _create_object(self, x_start, y_start, theta_start):
        ...
        # Draw orientation arrow for the object (red arrow)
        if self.graphics:
            arrow_length = self.shape_object.get_contact_radius() + 0.02
            end_x = x_start + arrow_length * np.cos(theta_start)
            end_y = y_start + arrow_length * np.sin(theta_start)
            pb.addUserDebugLine(
                lineFromXYZ=[x_start, y_start, z_position + 0.01],
                lineToXYZ=[end_x, end_y, z_position + 0.01],
                lineColorRGB=[1, 0, 0],  # Red for object orientation
                lineWidth=3
            )
